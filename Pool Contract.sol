...
* @dev This is a Staking pool 1 contract , 
* a part of the protocol where stakers are rewarded in tokens 
* when they make stakes of liquidity tokens from the liquidity pool.
...
//======================================ADMINSTRATION=========================================//
    ...
    modifier onlyTypeBToken() {
        require(msg.sender == TypeBtoken, "Authorization: only token contract can call");
        _;
    }
    
	 function tokenconfigs(address _typeBtoken, address _oracleindex) public onlyCreator returns (bool success) {
	    require(_typeBtoken != _oracleindex, "Insertion of same address is not supported");
	    require(_typeBtoken != address(0) && _oracleindex != address(0), "Insertion of address(0) is not supported");
        TypeBtoken = _typeBtoken;
	...
	require(TypeBtoken != address(0) && OracleIndexFunds != address(0), "Pool addresses are not yet setup");
...
//======================================ACTION CALLS=========================================//	
	...
	function TakeDividends() public returns (uint256) {
		...
		require(IERC20(TypeBtoken).transfer(msg.sender, _dividends), "Transaction Failed");    // Transfer dividends to msg.sender
    ...
    function scaledToken(uint _amount) external onlyTypeBToken returns(bool){
